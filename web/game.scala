...
/**
 * A tiny snake implementation.
 * Run the example, then click on the game canvas.
 * Press an arrow key to start, and use the arrows to play.
 * Collect the red apples - the game never ends!
 * Modified from: https://github.com/davesmith00000/snake-in-5-minutes-port
 */
@JSExportTopLevel("IndigoGame")
object Snake extends IndigoSandbox[Unit, SnakeModel]:

  val config: GameConfig         = GameConfig.default.withViewport(400, 400).withFrameRateLimit(FPS(15))
  ...
  def initialModel(startupData: Unit): Outcome[SnakeModel] =
    Outcome(SnakeModel(Point(10, 10), 20, Point(15, 15), Point.zero, Batch.empty, 5))
...
final case class SnakeModel(head: Point, gridSize: Int, apple: Point, direction: Point, trail: Batch[Point], tail: Int)